...
function showForm() 
{
	...
	if (!empty($Variables->PDFGen))
	{
		...
		// Calculator's header
		$pdf->SetHeaderMode();
		$pdf->Cell(190, 10, $lang['Calc2Title'], 1, 1, 'C', 1, '');
		...
		$pdf->Cell(40, 6, $lang['Currency'] . $AmortizationTable->TotalInterestPaid, 1, 0, 'C', 1, '');
		$pdf->Ln();
		
		$pdf->Cell(45, 6, $lang['Amount'], 1, 0, 'R', 1, '');
		$pdf->Cell(45, 6, $lang['Currency'] . $vars['Amount'], 1, 0, 'C', 1, '');
		$pdf->Cell(5, 6, ' ');
		$pdf->Cell(55, 6, $lang['PrincipalApplied'], 1, 0, 'R', 1, '');
		$pdf->Cell(40, 6, $lang['Currency'] . $AmortizationTable->TotalPrincipalApplied, 1, 0, 'C', 1, '');
		$pdf->Ln();
		...
		$pdf->Cell(40, 6, $lang['Currency'] . $AmortizationTable->TotalRemainingBalance, 1, 0, 'C', 1, '');
		$pdf->Ln();	
		...
		$pdf->SetTableMode(true);
		$pdf->Cell(90, 6, $lang['MonthsAlreadyPaid'], 1, 0, 'C', 1, '');
		$pdf->SetTableMode();
		$pdf->Ln();
		
		$pdf->Cell(45, 6, $lang['MonthsPaid'], 1, 0, 'R', 1, '');
		$pdf->Cell(45, 6, $vars['PaidPeriods'] . ' ' . $lang['Months'], 1, 0, 'C', 1, '');
		$pdf->Ln();
	...
	else 
	{
		...
		$content = $smarty->fetch($Variables->Template.'/form2.tpl');
